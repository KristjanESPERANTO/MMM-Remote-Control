...
function monitorControl (action, config, options, res, checkForExecError, sendSocketNotification) {
  ...
  const monitorOnCommand = config?.customCommand?.monitorOnCommand || "wlr-randr --output HDMI-A-1 --on";
  const monitorOffCommand = config?.customCommand?.monitorOffCommand || "wlr-randr --output HDMI-A-1 --off";
  const monitorStatusCommand = config?.customCommand?.monitorStatusCommand || "wlr-randr | grep -q 'Enabled: yes' && echo 'true' || echo 'false'";
